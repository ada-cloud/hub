...
const { server, served } = require("../message");
const { writeResponseMessage, writeResponse } = require("../helper");
...
class Client {
        ...
        return new Promise(resolve => {
            ...
            let getData = () => {
                Promise.resolve().then(() => this.cacheProvider.getPort(this.id)).then(info => {
                    if (info) {
                        let { topic, data } = info;
                        resolve(new served.BroadcastMessage(topic, data));
                    } else {
                        resolve(null);
                    }
                }).catch(e => {
    ...
    excute() {
        if (this._response) {
            ...
            return ps.then(() => {
                ...
                if (result.length > 0) {
                    writeResponse(this._response, 200, result.map(a => a.getInfo()));
                    this._response = null;
                    this._isRunning = false;
                } else {
                    this._isRunning = false;
    ...
    timeout() {
        if (this._response) {
            writeResponseMessage(this._response, 200, new server.TimeoutMessage());
            this._response = null;
        }
    }
}
